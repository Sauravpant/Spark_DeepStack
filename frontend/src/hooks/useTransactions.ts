...
import type { CreateTransactionPayload, UpdateCreditSalePayload, CreditStatus } from '@/types';
...
export function useCreateTransaction(shopId: string) {
  return useMutation({
    mutationFn: (payload: CreateTransactionPayload) => createTransaction(shopId, payload),
    onSuccess: (newTx) => {
...
export function useUpdateCreditSale(shopId: string) {
  return useMutation({
    ...
    onSuccess: (updated) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['dashboard', shopId] });
      toast.success(
        updated.status === 'paid' ? 'Payment recorded! Balance updated.' : 'Credit sale updated'
      );
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.message || 'Failed to update credit sale');
    },
  });
}
