  ...
  return (
    <Badge
      className={cn('text-xs font-semibold border-0 capitalize', {
        'bg-emerald-100 text-emerald-700': status === 'completed' || status === 'paid',
...
type CartLine = { product_id: string; quantity: number; name: string; price: number };

export default function Transactions() {
  const { activeShop } = useAuth();
  const shopId = activeShop?.id ?? '';
  const { data: transactions, isLoading, isError, refetch } = useTransactions(shopId);
  ...
  const cartTotal = cart.reduce((s, l) => s + l.price * l.quantity, 0) - discount;
            ...
            <div className="flex gap-2">
              {(['all', 'unpaid', 'overdue', 'paid'] as const).map((s) => (
                <Button
                  ...
                  className={
                    creditFilter === s ? 'bg-red-600 hover:bg-red-700 text-white' : ''
                  }
                    ...
                    {rows.map((sale) => (
                      <tr key={sale.id}>
                        <td className="px-4 py-3 font-semibold text-slate-800">
                          {formatCurrency(sale.credit_amount)}
                        </td>
                        <td className="px-4 py-3 text-sm text-slate-600">{sale.due_date}</td>
                        <td className="px-4 py-3">
                          <Badge
                            className={cn('border-0 capitalize', {
                              'bg-amber-100 text-amber-700': sale.status === 'unpaid',
                              'bg-red-100 text-red-700': sale.status === 'overdue',
                              'bg-emerald-100 text-emerald-700': sale.status === 'paid',
                            })}
                          >
                            {sale.status}
                          </Badge>
          ...
          {selected && (
            <>
              ...
              <div className="space-y-4 mt-5">
                <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                  ...
                  {selected.notes && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-slate-800">{selected.notes}</span>
                    </div>
                  )}
                </div>
                  ...
                  {selected.items.map((item) => (
                    ...
                    >
                      <span className="text-slate-700">
                        {productMap.get(item.product_id) ?? item.product_id.slice(0, 8)} ×{' '}
                        {item.quantity}
                      </span>
                ...
                {selected.credit_sale && selected.credit_sale.status !== 'paid' && (
                  <Button
      ...
      >
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={submitSale} className="space-y-4">
            <div className="grid grid-cols-3 gap-2">
              {(['cash', 'qr', 'credit'] as PaymentType[]).map((type) => (
                <Button
                  ...
                  className={
                    paymentType === type ? 'bg-red-600 hover:bg-red-700 text-white' : ''
                  }
            ...
            <div className="space-y-2">
              <Label>Customer {paymentType === 'credit' ? '(required)' : '(optional)'}</Label>
              <select
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" variant="outline" onClick={addProductToCart}>
                  Add
                </Button>
              </div>
            </div>
              ...
              <div className="space-y-2 border rounded-lg p-3">
                {cart.map((line) => (
                  <div key={line.product_id} className="flex items-center gap-2 text-sm">
                    <span className="flex-1 font-medium text-slate-800">{line.name}</span>
                    <Input
                      type="number"
                      min={1}
                      className="w-16 h-8"
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setCart((prev) => prev.filter((l) => l.product_id !== line.product_id))
                      }
                    >
